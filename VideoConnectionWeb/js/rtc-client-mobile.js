...
class RtcClient {
    ...
    async join() {
        ...
        try {
            ...
            try {
                ...
                this.localStream_.setVideoProfile((this.userId_ == $('#sendUserIDforZJR').val()) ? '720p' : '480p');
    ...
    async SubscribeTheStream(userid, divid) {
        ...
        } else if (userid == this.userId_ && this.localStream_ != null && $('#sendUserIDforZJR').val() != userid) {
    ...
    UNSubscribeTheStream() {
        ...
        //当自己不在大视频显示时，停止播放
        if ($('#sendUserIDforZJR').val() != this.userId_ && (this.localStream_ == null ? false : this.localStream_.isPlaying_) && GetStreamDiv(this.userId_) == 'no') {
            this.localStream_.stop();
