...
function AddAllUserToUserList() {
    ...
    //大于五个更新小视频页面
    if (columnNumber == 0 && rowsNumber == 0) {
        if (listnum > 5) {
            $("#video-grid").css('width', '20%');
            $("#video-grid").css('grid-template-columns', 'repeat(2, 1fr)');
            $("#video-grid").css('grid-template-rows', 'repeat(5, 20 %)');
        } else {
            $("#video-grid").css('width', '12%');
            $("#video-grid").css('grid-template-columns', 'repeat(1, 1fr)');
            $("#video-grid").css('grid-template-rows', 'repeat(5, 20 %)');
        }
    }
    for (var i = 0; i < UserList.length; i++) {
        ...
        if ($('#IsZCR').val() == true) {
            let str = '';
            str += '<input type="image" onclick="ClickMessgeForOne(^' + UserList[i].ID + '&)" style=" width: 28px; height: 28px; margin-left: 10px;" value="" src="./img/sf_ic_message_black.png" />';
            str = str.replace("^", "'");
            str = str.replace("&", "'");
            str += '<input type="image" onclick="ClickClose(^' + UserList[i].ID + '&)"style=" width: 28px; height: 28px; margin-left: 10px;" value="" src="./img/sf_menu_close_red.png" />';
            str = str.replace("^", "'");
            str = str.replace("&", "'");
            str += '<div ><button class="zcrbtn" onclick="ClickZCR(^' + UserList[i].ID + '&)">设置主讲人</button>';
            str = str.replace("^", "'");
            str = str.replace("&", "'");
            str += '</div>';
            let memberElm2 = $('#' + UserList[i].ID);
            memberElm2.find('div.qtdivZCR').html(str);
            memberElm2.find('div.qtdivZCR').css('display', 'flex');
        }
    }
}
//用户加入房间
...
function NextPage(pagenumber) {
    if ((pagenumber > userPage) || (pagenumber == 1)) {
        ...
        BDuser(((userPage - 1) * allCount), remainderPage);
    } else {
        BDuser((pagenumber * allCount), allCount);
    }
}
