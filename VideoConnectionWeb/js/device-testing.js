...
function getDeviceConnectInfo() {
    ...
    if (!(hasCameraDevice && hasMicDevice && hasVoiceDevice)) {
        connectInfo = `未检测到${hasCameraDevice ? '' : '【摄像头】'}${hasVoiceDevice ? '' : '【扬声器】'
...
async function updateCameraDeviceList() {
    ...
    let newCameraId = $('#camera-select')
        .children('option:selected')
        .val();
    if (newCameraId != '' && newCameraId != undefined) {
        cameraId = newCameraId;
        rtc.ChangeCameraId();
    }
...
async function testUplinkNetworkQuality() {
    ...
    await window.uplinkClient.join({ roomId: 1846464 }); // 加入用于测试的房间 1846464
