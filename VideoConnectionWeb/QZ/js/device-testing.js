...
async function deviceTestingInit() {
  ...
  // 测试tab页切换
  $("#camera-testing, #voice-testing, #mic-testing, #network-testing").on(
    "click",
    function () {
...
async function getDevicesInfo() {
  ...
  // hasNetworkConnect = isFileProtocol && navigator.onLine;
  ...
  const intervalId = setInterval(() => {
    if (countDown === 0) {
      ...
      $("#uplink-network")
        ...
        .text(
          NETWORK_QUALITY[
            String(networkTestingResult.average.uplinkNetworkQuality)
          ]
        );
      ...
      $("#downlink-network")
        ...
        .text(
          NETWORK_QUALITY[
            String(networkTestingResult.average.downlinkNetworkQuality)
          ]
        );
...
async function testUplinkNetworkQuality() {
  ...
  window.uplinkClient.on("network-quality", async (event) => {
    ...
    $("#network-rtt").removeClass("network-loading").text(`${rtt}ms`);
...
async function testDownlinkNetworkQuality() {
  ...
  window.downlinkClient.on("stream-added", async (event) => {
    await window.downlinkClient.subscribe(event.stream, {
      audio: true,
      video: true,
    });
    window.downlinkClient.on("network-quality", (event) => {
      ...
      console.log("----------------------------------");
      console.log(NETWORK_QUALITY[String(downlinkNetworkQuality)]);
      console.log("----------------------------------");
    });
  });
...
function showTestingReport() {
  ...
  $("#rtt-result")
    ...
    .css(
      "color",
      `${Number(networkTestingResult.average.rtt) > 150 ? "red" : "green"}`
    );
  ...
  $("#uplink-network-quality-result")
    ...
    .css(
      "color",
      `${
        Number(networkTestingResult.average.uplinkNetworkQuality) > 3
          ? "red"
          : "green"
      }`
    );
  ...
  $("#downlink-network-quality-result")
    ...
    .css(
      "color",
      `${
        Number(networkTestingResult.average.downlinkNetworkQuality) > 3
          ? "red"
          : "green"
      }`
    );
