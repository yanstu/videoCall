...
function getDeviceType() {
  ...
  switch (osType) {
    case "desktop":
      deviceType =
        osName.indexOf("Mac OS") > -1
          ? DEVICE_TYPE_ENUM.DESKTOP_MAC
          : DEVICE_TYPE_ENUM.DESKTOP_WIN;
      break;
    case "mobile":
      deviceType =
        osName === "iOS"
          ? DEVICE_TYPE_ENUM.MOBILE_IOS
          : DEVICE_TYPE_ENUM.MOBILE_ANDROID;
      break;
    default:
      break;
  }
...
function getRecommendBrowserInfo() {
  ...
  switch (deviceType) {
    ...
    case DEVICE_TYPE_ENUM.DESKTOP_WIN:
      recommendBrowserInfo =
        " Windows 设备请使用 Chrome, Firefox 56+ 或 Edge 80+ 浏览器打开链接";
      break;
    ...
    default:
      recommendBrowserInfo =
        "建议下载最新版Chrome浏览器（http://www.google.cn/chrome/）打开链接";
      break;
  }
...
function isFirefoxM56() {
  if (
    deviceType === DEVICE_TYPE_ENUM.DESKTOP_WIN ||
    deviceType === DEVICE_TYPE_ENUM.DESKTOP_MAC
  ) {
